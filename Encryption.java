...
// The program consists of a single Java class called "Encryption".
//  The class extends the JFrame class and implements the ActionListener 
//  interface to handle GUI events. The GUI consists of a text area for entering the message,
//   buttons for opening an image, embedding the message, sending the image, saving the image, and 
//   resetting the interface. The original image and the embedded image are displayed side by side in 
//   separate scroll panes.

// The program uses the ImageIO class to read and write image files, and the JOptionPane class to 
// display error messages. The LSB algorithm is used to embed the message
//  by replacing the least significant bit of each pixel's RGB color 
//  component with the corresponding bit of the message. 
// The program checks that the user has selected an image and entered 
// a message before attempting to embed the message.
